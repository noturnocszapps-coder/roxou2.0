...
export default function AdminDriverManagement({
  ...
  const handleBulkAction = async (status: "approved" | "rejected") => {
    ...
    if (
      !confirm(
        ...
        } todos os ${driversToUpdate.length} motoristas filtrados?`
      )
    ) {
      return;
    }
  ...
  const inReviewCount = drivers.filter(
    (d) => d.verification_status === "in_review"
  ).length;
  ...
  const tabs = [
    {
      ...
      count: pendingCount,
      icon: Clock,
      color: "text-roxou-primary",
      bg: "bg-roxou-primary/10",
    },
    {
      id: "in_review",
      label: "Em Análise",
            ...
            >
              <tab.icon
                className={`w-3.5 h-3.5 ${
                  filter === tab.id ? tab.color : "text-roxou-text-muted"
                }`}
              />
              {tab.label}
              <span
      ...
      {filteredDrivers.length > 0 && filter === "pending" && (
        <div className="flex flex-col sm:flex-row items-center gap-4 p-5 rounded-[32px] bg-roxou-primary/5 border border-roxou-primary/10 animate-in fade-in slide-in-from-top-2">
          <div className="flex items-center gap-3 flex-grow">
            ...
            <div>
              <p className="text-[10px] font-black text-roxou-primary uppercase tracking-widest">
                Ações em Massa
              </p>
              <p className="text-xs text-roxou-text-muted font-medium">
                {filteredDrivers.length} motoristas selecionados
              </p>
            </div>
          </div>
      ...
      <div className="space-y-6">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <p className="text-xs font-black text-roxou-primary uppercase tracking-widest animate-pulse">
              Sincronizando dados...
            </p>
          </div>
            ...
            >
              <div className="p-6 sm:p-10 space-y-8">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                  <div className="flex items-center gap-5 sm:gap-8">
                    <div className="relative">
                      ...
                      <div
                        className={`absolute -bottom-2 -right-2 w-8 h-8 rounded-xl border-2 border-roxou-surface z-20 flex items-center justify-center shadow-lg ${
                          driver.verification_status === "approved"
                            ? "bg-emerald-500"
                            : driver.verification_status === "rejected"
                            ? "bg-red-500"
                            : driver.verification_status === "in_review"
                            ? "bg-amber-500"
                            : "bg-roxou-primary"
                        }`}
                      >
                    ...
                    <div className="space-y-1.5">
                      <h4 className="font-display font-black text-2xl sm:text-3xl leading-tight text-white tracking-tight">
                        {driver.full_name}
                      </h4>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="p-5 rounded-3xl bg-roxou-bg/40 border border-roxou-border/50 space-y-3">
                    <div className="flex items-center gap-2 text-roxou-text-muted">
                    ...
                    <p className="text-base font-bold text-white/90">
                      {driver.vehicle_model || "Não informado"}
                    </p>
                  </div>
                  ...
                  <div className="p-5 rounded-3xl bg-roxou-bg/40 border border-roxou-border/50 space-y-3">
                    ...
                    <p className="text-base font-bold text-white/90 uppercase tracking-wider">
                ...
                <div className="pt-6 border-t border-roxou-border/50 space-y-6">
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
                    <div className="flex-grow relative">
                      <Filter className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-roxou-text-muted" />
                      <input
                        type="text"
          ...
          <div className="text-center py-24 sm:py-32 p-10 rounded-[48px] border-2 border-dashed border-roxou-border bg-roxou-surface/20">
            <div className="w-20 h-20 bg-roxou-surface rounded-[28px] flex items-center justify-center mx-auto mb-6 border border-roxou-border shadow-2xl">
              {filter === "pending" ? (
                <Clock className="w-10 h-10 text-roxou-text-muted/30" />
              ) : filter === "approved" ? (
                <ShieldCheck className="w-10 h-10 text-roxou-text-muted/30" />
              ) : filter === "in_review" ? (
                <AlertCircle className="w-10 h-10 text-roxou-text-muted/30" />
              ) : (
            ...
            <p className="text-sm text-roxou-text-muted/60 max-w-xs mx-auto font-medium">
              {filter === "pending"
                ? "Quando novos motoristas se cadastrarem, eles aparecerão aqui para sua revisão."
