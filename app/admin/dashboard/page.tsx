    ...
    <div className="min-h-screen bg-roxou-bg pb-20">
      ...
      <header className="glass sticky top-0 z-40 py-3 sm:py-4 px-4 sm:px-6">
        <div className="max-w-5xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <span className="text-lg sm:text-xl font-display font-bold tracking-tighter">ROXOU ADMIN</span>
          </div>
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 pt-6 sm:pt-10 space-y-8 sm:space-y-12">
        ...
        <div className="space-y-1 sm:space-y-2">
          <h1 className="text-3xl sm:text-4xl font-display font-black text-white tracking-tight">Roxou Admin</h1>
          <p className="text-sm sm:text-base text-roxou-text-muted font-medium">Gerencie motoristas e acompanhe a atividade da plataforma</p>
        </div>

        {/* Stats Grid */}
        <div className="space-y-4 sm:space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-[10px] sm:text-sm font-black uppercase tracking-[0.2em] sm:tracking-[0.3em] text-roxou-primary">Visão Geral da Plataforma</h2>
            <RefreshDashboard />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            ...
            <div className="p-6 sm:p-8 rounded-[32px] sm:rounded-[40px] bg-roxou-surface border-2 border-roxou-primary/30 relative overflow-hidden group sm:col-span-2 violet-glow-sm">
              ...
              <div className="flex items-center gap-3 mb-3 sm:mb-4">
                ...
                <p className="text-[9px] sm:text-[10px] text-roxou-text-muted uppercase font-bold tracking-widest">Total Motoristas</p>
              </div>
              <div className="flex flex-col">
                <p className="text-4xl sm:text-5xl font-display font-black text-white">{totalDrivers || 0}</p>
                {totalDrivers === 0 && (
                  <p className="text-[9px] sm:text-[10px] font-bold text-roxou-text-muted/60 uppercase tracking-widest mt-1">Aguardando atividade</p>
                )}
              </div>
            </div>
            ...
            <div className="p-6 sm:p-8 rounded-[32px] sm:rounded-[40px] bg-roxou-surface border border-roxou-border relative overflow-hidden group">
              ...
              <p className="text-[9px] sm:text-[10px] text-roxou-text-muted uppercase font-bold tracking-widest mb-2">Pedidos Totais</p>
              <p className="text-2xl sm:text-3xl font-display font-bold text-white/80">{totalRequests || 0}</p>
              {totalRequests === 0 && (
                <p className="text-[9px] sm:text-[10px] font-bold text-roxou-text-muted/40 uppercase tracking-widest mt-1">Nenhum dado ainda</p>
              )}
            </div>
            ...
            <div className="p-6 sm:p-8 rounded-[32px] sm:rounded-[40px] bg-roxou-surface border border-red-500/30 relative overflow-hidden group shadow-[0_0_20px_rgba(239,68,68,0.1)]">
              ...
              <p className="text-[9px] sm:text-[10px] text-red-500/80 uppercase font-bold tracking-widest mb-2">Denúncias</p>
              <p className="text-2xl sm:text-3xl font-display font-bold text-red-500">{reports?.length || 0}</p>
              {(!reports || reports.length === 0) && (
                <p className="text-[9px] sm:text-[10px] font-bold text-red-500/40 uppercase tracking-widest mt-1">Tudo limpo</p>
              )}
            </div>
            ...
            <div className="p-6 sm:p-8 rounded-[32px] sm:rounded-[40px] bg-roxou-surface border border-roxou-border relative overflow-hidden group sm:col-span-2 lg:col-span-1">
              ...
              <p className="text-[9px] sm:text-[10px] text-roxou-text-muted uppercase font-bold tracking-widest mb-2">Pendentes</p>
              <p className="text-2xl sm:text-3xl font-display font-bold text-yellow-500">{pendingCount}</p>
              {pendingCount === 0 && (
                <p className="text-[9px] sm:text-[10px] font-bold text-roxou-text-muted/40 uppercase tracking-widest mt-1">Sem pendências</p>
              )}
            </div>
          </div>
        </div>
            ...
            <div className="space-y-4">
              {reports && reports.length > 0 ? (
                ...
                <div className="text-center py-12 sm:py-20 p-6 sm:p-10 rounded-[32px] sm:rounded-[40px] border-2 border-dashed border-roxou-border bg-roxou-surface/20">
                  ...
                  <p className="text-[10px] sm:text-xs text-roxou-text-muted/60">Nenhuma denúncia recente.</p>
                </div>
              )}
            </div>
          </section>
